...
import com.denger.client.modules.mods.render.NoCameraClip;
    ...
    public void setup(IBlockReader p_216772_1_, Entity p_216772_2_, boolean p_216772_3_, boolean p_216772_4_, float p_216772_5_) {
        ...
        if (p_216772_3_) {
            if (p_216772_4_) {
                ...
                this.setRotation(this.yRot + prev, -this.xRot);
            }else{
                prev = fast(prev,0,10);
            }
            double temp;
            if (getInstance.getRegisterModule().isEnable(NoCameraClip.class)){
                temp = NoCameraClip.getDistance(NoCameraClip.distancia.getVal());
            }else {
                temp = NoCameraClip.getDistance(this.getMaxZoom(4));
            }
            this.move(-temp, 0.0D, 0.0D);
